...
class ShareLetterController extends Controller
{
        ...
        return datatables($model)
            ...
            ->addColumn('action', function ($model) {
                ...
                return "
                    ...
                    <a href='". route('admin.undangan.sent-status', ['id' => cryptId($model->id), 'status' => 'sending']) ."' class='btn btn-sm btn-". (in_array($model->status, ['sended', 'sending']) ? '' : 'outline-') ."primary' onclick='handleSendingInv(event)' data-status='". $model->status ."'><i class='ti ti-share'></i></a>
                ";
            })
            ->rawColumns(['action'])
            ->toJson();
    ...
    public function sentStatus (Request $request, $id)
    {
        ...
        $urlEncode = 'Assalamualaikum Warahmatullahi Wabarakatuh' . "%0D%0A%0D%0A";
        $urlEncode .= 'Dengan memohon rahmat dan ridho Allah SWT. kami bermaksud mengundang Bapak/Ibu/Saudara/i '. $model->receiver_name .' untuk hadir dalam acara pernikahan kami.' . "%0D%0A%0D%0A";
        $urlEncode .= 'Untuk Info Detail Acara, Lokasi, dan Waktu Lebih Lengkap bisa akses link undangan online berikut : ' . "%0D%0A%0D%0A";
